package com.cong.sqldog.sqlanalyze.extract;
...
public class SqlExtract {
    ...
    public static SqlExtractResult extract(StatementHandler statementHandler) {
        ...
        if (delegateMetaObject.getValue(ExtractConstant.DELEGATE) == null) {
            logger.warn("sql analysis get delegate null error,{}", statementHandler.getBoundSql().getSql());
            return null;
        }
        if (!(delegateMetaObject.getValue(ExtractConstant.DELEGATE) instanceof PreparedStatementHandler)) {
            logger.info("sql analysis get delegate is not PreparedStatementHandler,{}", statementHandler.getBoundSql().getSql());
            return null;
        }
        PreparedStatementHandler preparedStatementHandler = (PreparedStatementHandler) delegateMetaObject.getValue(ExtractConstant.DELEGATE);
    ...
    private static boolean needAnalysis(String sqlId, String sqlType) {
        ...
        //检查间隔判断
        return CHECKED_ID_MAP.get(sqlId) == null || (System.currentTimeMillis() - CHECKED_ID_MAP.get(sqlId)) >= SqlAnalysisConfig.getCheckInterval();
    }
